(function () {
  ...
  function formatStationTime(ms) {
    return new Intl.DateTimeFormat("en-US", {timeZone:DISPLAY_TIME_ZONE,hour:"numeric",minute:"2-digit"}).format(new Date(ms));
  }
  ...
  function artForMovie(movie) {
    ...
    return "assets/channel-share.jpg";
  }
  ...
  function renderGuide() {
    els.guideDate.textContent = new Intl.DateTimeFormat("en-US", {timeZone:DISPLAY_TIME_ZONE,weekday:"long",month:"long",day:"numeric"}).format(new Date());
  ...
  function showStationCard(state) {
    ...
    if (state.segment.kind === "movie") {
      ...
      els.cardCountdown.textContent = "Cartoon source not connected yet";
      return;
    }
    if (state.segment.kind === "commercial") {
      els.cardLabel.textContent = "COMMERCIAL BREAK";
      els.cardTitle.textContent = state.segment.title;
      els.cardCountdown.textContent = `${formatDuration(state.movieReturnsIn)} until the cartoon returns`;
      return;
    }
    els.cardLabel.textContent = "Cartoon Network";
    els.cardTitle.textContent = state.segment.title;
    els.cardCountdown.textContent = `${formatDuration(state.segmentRemaining)} until the next cartoon`;
  }
  ...
  function tick() {
    ...
    loadMedia(state);
    try {
      localStorage.setItem("infinity_live_Cartoon-Network", JSON.stringify({title:state.block.movie.title, collection:state.block.movie.collection || "Cartoon Network", startsAtMs:state.block.startsAtMs, endsAtMs:state.block.endsAtMs, updatedAt:Date.now()}));
    } catch (_) {}
  }
  ...
  function localShareCredit(reference) {
    ...
    const profile = signedIn || parse("starquest_guest_profile_v1", {key:"__guest__", username:"Guest", tokens:0, shareCount:0, pendingShareCredits:0, shareEvents:[], ledger:[], watchHistory:[], watchPositions:{}, unlockedContent:{}});
  ...
  window.onYouTubeIframeAPIReady = function () {
    player = new YT.Player("player", {
      ...
      events:{
        ...
        onError:() => { if (loadedMovieVideoId) failedMovieVideoIds.add(loadedMovieVideoId); scheduleKey = ""; loadedKey = ""; loadedMovieVideoId = ""; setTimeout(tick, 250); }
      }
    });
  };
